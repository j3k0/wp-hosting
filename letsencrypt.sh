...
if [ "x$1" != "x--phpmyadmin-only" ]; then
    ...
    echo sudo certbot $STAGING --authenticator nginx --webroot-path $PROJECT/volumes/html --installer nginx --no-redirect -d $DOMAINS
    sudo certbot $STAGING --authenticator nginx --webroot-path $PROJECT/volumes/html --installer nginx --no-redirect -d $DOMAINS
    set -e
fi
...
if [ "x$1" != "x--web-only" ]; then
    ...
    sudo certbot $STAGING --authenticator nginx --webroot-path "$PHPMYADMIN_WEBROOT" --installer nginx --redirect -d $DOMAINS
fi
