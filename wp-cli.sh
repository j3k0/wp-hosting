...
if test -e $PROJECT/docker-compose.yml; then
    ...
    docker run --rm -it $NETWORK_ARG $VOLUMES_ARG -v $(pwd)/_scripts/wp:/usr/local/bin/wp -v $(which less):/usr/bin/less  --workdir /var/www/html --entrypoint wp --user=www-data wordpress --allow-root "$@"
else
